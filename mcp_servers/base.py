...
class BaseMCPServer(ABC):
    ...
    def _register_decorated_tools(self) -> None:
        ...
        for name, method in inspect.getmembers(self, inspect.ismethod):
            if hasattr(method, "_mcp_tool") and getattr(method, "_mcp_tool"):
                tool_name = getattr(method, "_mcp_tool_name")
                tool_description = getattr(method, "_mcp_tool_description")
                tool_category = getattr(method, "_mcp_tool_category")
                tool_tags = getattr(method, "_mcp_tool_tags")
    ...
    async def handle_tool_call(
        self, websocket: WebSocketServerProtocol, call: MCPToolCall
    ) -> None:
        """Handle a tool call.
