...
class ProgressTracker:
    ...
    def track_exercise_completion(
        self, user_id: str, exercise_id: str, score: int
    ) -> UserProgress:
    ...
    def set_current_tutorial(
        self, user_id: str, tutorial_id: Optional[str], section_id: Optional[str] = None
    ) -> UserProgress:
    ...
    def _check_achievements(self, progress: UserProgress) -> None:
        ...
        if (
            len(progress.completed_tutorials) >= 1
            and "first_tutorial" not in existing_achievement_ids
        ):
        ...
        if (
            len(progress.completed_tutorials) >= 5
            and "tutorial_master" not in existing_achievement_ids
        ):
            progress.achievements.append(
                Achievement(
                    id="tutorial_master",
        ...
        if (
            len(progress.exercise_scores) >= 10
            and "exercise_expert" not in existing_achievement_ids
        ):
            progress.achievements.append(
                Achievement(
                    id="exercise_expert",
    ...
    def _check_certificates(self, progress: UserProgress) -> None:
        ...
        if (
            all(tutorial_id in progress.completed_tutorials for tutorial_id in beginner_tutorials + intermediate_tutorials + advanced_tutorials)
            and beginner_tutorials and intermediate_tutorials and advanced_tutorials
    ...
    async def get_progress_summary(self, user_id: str) -> Dict:
        ...
        return {
            ...
            "tutorial_completion_percentage": (completed_tutorials / total_tutorials * 100) if total_tutorials > 0 else 0,
